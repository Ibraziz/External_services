...
def analyze_legal_document(data_dict, output_file=None):
    ...
    for i, chunk in enumerate(chunks, 1):
        ...
        print(f"Processing chunk {i}/{len(chunks)} (pages {page_nums[0]}-{page_nums[-1]})...")
        
        current_analysis = analyze_chunk(chunk, current_analysis)
        
        print(f"Summary: {current_analysis.summary[:80]}...")
        print(f"Keywords: {'; '.join([', '.join(page_kws) for page_kws in current_analysis.keywords])}\n")
        print(f"Suggested Questions: {', '.join(current_analysis.suggested_questions)}\n")
